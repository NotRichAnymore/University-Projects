...
void etmcargo::newCargoO() {

    QTextStream input(stdin); // this will allow input for Qstrings

    cargoOID = 0; // have set to auto increment in the database.
...
//try and catch to insert data if not display not connected
        try {
            if(database_connection.database_status())
            {
                query.prepare("INSERT INTO EtmCargoOData (CargoOID,FirstName,LastName,Username,Email,Password,UserAddress,StreetName,City,Postcode,PhoneNumber)"
                              "VALUES(:cargoOID,:firstName,:lastName,:usernameC,:userEmail,:passwordC,:userAddress,:streetName,:cityName,:postCode,:phoneNumber)");

                query.bindValue(0,cargoOID);
